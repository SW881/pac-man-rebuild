...
export function usePacmanGame() {
  ...
  //     // Update gameState from previous state (safe for setInterval). [web:39]
  //     setGameState((prev) => {
  //       if (
  //         checkIfPacEaten(
  //           prev.pac_man,
  ...
  //       // Food eat (cells is separate state; update it immutably)
  //       let ate = false;
  //       setCells((oldCells) => {
  //         const result = eatFoodAt(oldCells, nextPac.x, nextPac.y);
  //         ate = result.ate;
  //         return result.cells;
  //       });
